Weapons = function(Player) {
    ...
    var engine = Player.scene.getEngine();

    Player.scene.registerBeforeRender(function() {
        if (!_this.canFire) {
...
Weapons.prototype = {
    ...
    launchFire : function() {
        if (this.canFire) {
            ...
            var renderWidth = this.Player.engine.getRenderWidth(true);
            var renderHeight = this.Player.engine.getRenderHeight(true);
            
            var direction = this.Player.scene.pick(renderWidth/2,renderHeight/2);
    ...
    createRocket : function(playerPosition, direction) {
        ...
        var newRocket = BABYLON.Mesh.CreateBox("rocket", 1, this.Player.scene);
        ...
        newRocket.material = new BABYLON.StandardMaterial("textureWeapon", this.Player.scene);
        ...
        newRocket.registerAfterRender(function(){
            ...
            if(!meshFound || meshFound.distance < 10){
                ...
                if(meshFound.pickedMesh){
                    ...
                    var explosionRadius = BABYLON.Mesh.CreateSphere("sphere", 5.0, 20, Player.scene);
                    ...
                    explosionRadius.material = new BABYLON.StandardMaterial("textureExplosion", Player.scene);
